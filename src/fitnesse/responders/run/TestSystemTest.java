...
public class TestSystemTest {
	...
	public void testCommandPatternCSharp() throws Exception {
		...
		Descriptor defaultDescriptor2 = TestSystem.getDescriptor(
				specifiedPage.getData(), context.pageFactory, false);
		assertEquals("%m -r fitSharp.Slim.Service.Runner,fitsharp.dll %p",
				defaultDescriptor2.commandPattern);

		Descriptor defaultDescriptor3 = TestSystem.getDescriptor(
				specifiedPage.getData(), context.pageFactory, true);
		assertEquals("%m -r fitSharp.Slim.Service.Runner,fitsharp.dll %p",
				defaultDescriptor3.commandPattern);
	}
	...
	public void testCommandPatternJava() throws Exception {
		...
		Descriptor defaultDescriptor = TestSystem.getDescriptor(page.getData(),
				context.pageFactory, false);
		String sep = System.getProperty("path.separator");
		assertEquals("java -cp fitnesse.jar" + sep + "%p %m",
				defaultDescriptor.commandPattern);

		Descriptor debugDescriptor = TestSystem.getDescriptor(page.getData(),
				context.pageFactory, true);
		assertEquals(
		...
		Descriptor defaultDescriptor2 = TestSystem.getDescriptor(
				specifiedPage.getData(), context.pageFactory, false);
		assertEquals("java -specialParam -cp %p %m",
				defaultDescriptor2.commandPattern);

		Descriptor debugDescriptor2 = TestSystem.getDescriptor(
				specifiedPage.getData(), context.pageFactory, true);
		assertEquals("java -remoteDebug -cp %p %m",
				debugDescriptor2.commandPattern);
	}
	...
	public void testCommandPatternWithVariable() throws Exception {
		...
		Descriptor myDescriptor = TestSystem.getDescriptor(
				specifiedPage.getData(), context.pageFactory, false);
	...
	public void testRunnerCSharp() throws Exception {
		...
		Descriptor defaultDescriptor2 = TestSystem.getDescriptor(
				specifiedPage.getData(), context.pageFactory, false);
		assertEquals("..\\fitnesse\\fitsharp\\Runner.exe",
				defaultDescriptor2.testRunner);
		Descriptor defaultDescriptor3 = TestSystem.getDescriptor(
				specifiedPage.getData(), context.pageFactory, true);
		assertEquals("..\\fitnesse\\fitsharp\\runnerw.exe",
				defaultDescriptor3.testRunner);
	}
	...
	public void testRunnerDefault() throws Exception {
		...
		Descriptor defaultDescriptor2 = TestSystem.getDescriptor(
				page.getData(), context.pageFactory, false);
		assertEquals("fitnesse.slim.SlimService", defaultDescriptor2.testRunner);
		Descriptor defaultDescriptor3 = TestSystem.getDescriptor(
				page.getData(), context.pageFactory, true);
		...
		Descriptor specifiedDescriptor = TestSystem.getDescriptor(
				specifiedPage.getData(), context.pageFactory, true);
